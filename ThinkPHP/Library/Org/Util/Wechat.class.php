...
class Wechat {
    ...
    public function __construct($options) {
        ...
        $this->mpid = isset($options['mpid']) ? $options["mpid"] : false;
        $this->isauth = isset($options['isauth']) ? $options["isauth"] : false;
        $this->component_appid = C('WEIMI.component_appid');
	    $this->component_appsecret = C('WEIMI.component_appsecret');
	    $this->component_access_token='';//getComponentAccessToken();
    }
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '') {
        ...
        if($this->isauth){
            $apiurl = "http://www".PANEL_DOMAIN.'/Service/Service/getAccessToken';
            $param["mpid"] = $this->mpid;
            $param["isapi"] = 1;
            $result = http($apiurl, $param);
            $result = (json_decode($result, true));
            if ($result ['status'] == 1) {
                $this->access_token = $result["msg"];
            }else{
...
// :TODO: Is there any more datatype we should be in the lookout for? (Object?)
    ...
    public function getMediaNew($media_id) {
        if (!$this->access_token && !$this->checkAuth())
            return false;
    ...
    public function getOauthRedirect($callback, $state = '', $scope = 'snsapi_userinfo') {
        ...
        if($this->isauth){
        	return self::OAUTH_PREFIX . self::OAUTH_AUTHORIZE_URL . 'appid=' . $this->appid . '&redirect_uri=' . urlencode($callback) . '&response_type=code&scope=' . $scope . '&state=' . $state . '&component_appid=' . $this->component_appid . '#wechat_redirect';
        }else{
    ...
    public function getOauthAccessToken() {
        ...
        if($this->isauth){
        $result = $this->http_get(self::API_BASE_URL_PREFIX . '/sns/oauth2/component/access_token?' . 'appid=' . $this->appid . '&secret=' . $appsecret . '&code=' . $code . '&grant_type=authorization_code&component_appid=' . $this->component_appid . '&component_access_token=' . $this->component_access_token . '');
        }else{
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::OAUTH_TOKEN_URL . 'appid=' . $this->appid . '&secret=' . $this->appsecret . '&code=' . $code . '&grant_type=authorization_code&component_appid=' . $this->component_appid . '&component_access_token=' . $this->component_access_token. '');
        }
        if ($result) {
        	//logdebug($result);
            $json = json_decode($result, true);
    ...
    public function getOauthRefreshToken($refresh_token) {
    	$result = $this->http_get(self::API_BASE_URL_PREFIX . self::OAUTH_REFRESH_URL . 'appid=' . $this->appid . '&grant_type=refresh_token&refresh_token=' . $refresh_token.'&component_appid=' . $this->component_appid . '');
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 发送媒体文件到服务器，返回media_id
     *
     * @param unknown $data
     * @param string $type:image,voice,video,thumb
     * @return boolean|array{"type":"TYPE","media_id":"MEDIA_ID","created_at":123456789}
     */
    public function sendMedia($data = null, $type = 'image') {
        if (!$this->access_token && !$this->checkAuth())
            return false;
        $url = self::UPLOAD_MEDIA_URL . self::MEDIA_UPLOAD . 'access_token=' . $this->access_token . '&type=' . $type;
        $ch = curl_init();
...
class Prpcrypt {
    ...
    public function decrypt($encrypted, $appid) {
        ...
        try {
            ...
            //logdebug("xml_content-------------".$xml_content);
            $from_appid = substr($content, $xml_len + 4);
            //logdebug("from_appid-------------".$from_appid);
