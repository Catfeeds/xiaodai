...
class Rbac {
    ...
    static public function getAccessList($authId) {
        ...
        foreach($apps as $key=>$app) {
            ...
            $special[strtoupper($appName)]   =  array();
            $sql    =   "select node.id,node.name from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
                    $table['access']." as access ,".
            ...
            foreach($modules as $key=>$module) {
                ...
                $moduleName = $module['name'];
                $arrUrl=null;
                $sql    =   "select node.id,node.name,node.url from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
                    $table['access']." as access ,".
                    $table['node']." as node ".
                ...
                foreach ($rs as $a){
                		//增加四级
                		$sql1    =   "select node.id,node.name,node.url from ".
                				$table['role']." as role,".
                				$table['user']." as user,".
                				$table['access']." as access ,".
                				$table['node']." as node ".
                				"where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=4 and node.pid={$a['id']}";// and node.status=1";
                		$rs1 =   $db->query($sql1);
                		if($rs1){
	                		//$action = array();
	                		foreach ($rs1 as $a1){
	                			$special[strtoupper($appName)][strtoupper($a['name'])][strtoupper($a1['name'])]	 =	 $a1['id'];
	                			if(!isN($a1['url'])){
	                				$a1['url']=strtoupper($a1['url']);
	                				$arrUrl1=str2arr($a1['url'],'/');
	                				$special[strtoupper($appName)][$arrUrl1[0]][$arrUrl1[1]]=$a['id'];
	                				$arrUrl1=null;
	                			}
	                		}
                		}else{
                			$action[$a['name']]	 =	 $a['id'];
                			if(!isN($a['url'])){
                				$a['url']=strtoupper($a['url']);
                				$arrUrl=str2arr($a['url'],'/');
                				$special[strtoupper($appName)][$arrUrl[0]][$arrUrl[1]]=$a['id'];
                				$arrUrl=null;
                			}
                		}
        ...
        $access=array_merge_recursive( $special,$access); 
