...
<script type="text/javascript" src="/Public/Admin/lib/zclip/jquery.zclip.min.js"></script>
<script language="javascript">
$(function(){
	
	$('.zcopy').zclip({
		path: '/Public/Admin/lib/zclip/ZeroClipboard.swf',
		copy: function(){
			var $v=$(this).attr("rel");
			return $v;
		},
 	...
 	$.setActive=function($id,$status){
		var $url="<?php echo U($control.'/activity');?>?pid="+$id+"&active="+$status;	
		$.ajax({
			url:$url,
            ...
            <td><div class="col-md-6">
                <div class="input-group">
                  ...
                  <select class="btn" name="searchtype" id="searchtype">
                    ...
                    <?php if(($searchtype) == "1"): ?><option value="1" selected="selected">活动描述</option>
                      <?php else: ?>
                      <option value="1">活动描述</option><?php endif; ?>
                  </select>
    ...
    <table class="table  table-first-column-number display full table-striped custom-table vm">
      ...
      <tbody>
        <?php if(is_array($contentlist)): $i = 0; $__LIST__ = $contentlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            ...
            <td><?php echo ($vo['timefrom']); ?> 至 <?php echo ($vo['timeto']); ?></td>
            <td><?php echo get_activity_count($control,$vo['id'],'history');?></td>
            <td>
            <?php if(($vo["isdefault"]) == "1"): ?><a href="javascript:void(0);" onclick="$.setActive(<?php echo ($vo["id"]); ?>,0);" class="btn btn-link" style="color:#7BB13F;"><i class="fa fa-check-circle"></i> 是</a>
            <?php else: ?>
            <a href="javascript:void(0);" onclick="$.setActive(<?php echo ($vo["id"]); ?>,1);" class="btn btn-link"><i class="fa fa-times-circle"></i> 否</a><?php endif; ?>
            </td>
             <td><a href="javascript:void(0);" rel="<?php echo get_activity_url($control,$vo['id']);?>" class="zcopy" style="position:relative" >复制</a></td>
        ...
        <tr>
          <td colspan="9"><?php if(!empty($page)): ?><div class="pull-left custom-footer">
