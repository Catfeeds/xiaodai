...
class WheelController extends BaseController {
	...
	public function editActivity($id = 0) {
		...
		if (IS_POST) {
			...
			if ($id) {
				...
				// 清除缓存
				get_cache_activity ( 'wheel', $id, true );
	...
	public function prize($actid = '', $searchtype = '', $keyword = '', $pid = '', $status = '', $p = 1) {
		...
		if (! is_numeric ( $actid )) {
			$actid = get_current_activity_id ( 'wheel' );
		} else {
			get_current_activity_id ( 'wheel', $actid );
		}
		if($actid){
			$where ['actid'] = $actid;
		}
		...
		$actlist = get_cache_list ( 'wheel_activity', '','id,title,sort');
	...
	public function history($actid = '', $searchtype = '', $keyword = '',$timefrom = '', $timeto = '', $p = 1) {
		...
		$actlist = get_cache_list ( 'wheel_activity', '', 'id,title' );
		$this->assign ( "actlist", $actlist );
		$this->assign ( 'actid', $actid );
	...
	public function record($actid = '', $prizeid='', $searchtype = '', $keyword = '',$timefrom = '', $timeto = '', $status='',$p = 1) {
		...
		$where1=array();
		$where1 ['actid'] = $actid;
		$prizelist = M('wheel_prize')->where($where1)->field('id,title,prize')->cache(true)->order('sort asc')->select();
