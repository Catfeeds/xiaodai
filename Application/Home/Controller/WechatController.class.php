	...
	public function index() {
		...
		switch ($type) {
			case $wechat::MSGTYPE_TEXT :
				...
				$this->addkefumesg($wechat->getRevFrom(),$msg);
				break;
...
//						$this->unsubscribe($wechat->getRevFrom());
	...
	private function addkefumesg($openid,$msg){
...
//		$member=M('member')->where(array('id'=>$memberid))->find();
		...
		if($exist){
			...
			$contents[$count]['type']='answ';
			$contents[$count]['headimgurl']=$member['headimgurl'];
			$contents[$count]['content']=$content;
		}else{
			$contents=array();
			$contents[0]['addtime']=$addtime;
			$contents[0]['type']='answ';
			$contents[0]['headimgurl']=$member['headimgurl'];
...
//		}
//		$result['status']=1;
//		$result['headimgurl']=$member['headimgurl'];
