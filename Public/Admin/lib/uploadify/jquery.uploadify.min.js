...
$(function(){
	$.rendUploader("#btnUpload");
	$.rendUploader("#btnUploadatt");
...
(function(){
	$.rendUploader=function(id){
		 $(id).uploadify({
		   ...
		   "buttonText"      : '<i class="fa fa-upload"> 上传</i>',
		...
		function uploadFilefile_id( file, data, response){  
			//var data='{"status":1,"info":"zheshiceshixinxi","data":"fadsjkfdsaklfjdslkajfklds"}';
		   ...
		   if(data.status){
			var name = id;
			$(name).parent().prev("input:text").val(data.info);
	...
	$.rendUploaderHover=function(){
		 
		 $(".uploadify").unbind("hover").hover(function(event){ 
		 var o=$(this).parent().prev("input");
		 var img=o.val();
		 ...
		 if($.trim(img)!=""){  
			 if($("#hoverimg").length==0){
				...
				$("#hoverimg").css({"left":o.offset().left,"top":o.offset().top+31}).show()
				}
			 }
		 },function(){
			  $("#hoverimg").hide();
	...
	$.rendUploaderMaterial=function(id){
		 $(id).uploadify({
		   ...
		   "width"           : 50,
		   'removeTimeout'	  : 1,
		   "onUploadSuccess" : uploadFilefile_id1							   
		   });
		   
		function uploadFilefile_id1( file, data, response){  
		var data = $.parseJSON(data);
		   if(data.status){
			var $obj=$(id).prev("input"); 
		 	$obj.val(data.info);
			var name = $obj.attr("data-image");
			$(name).attr("src",data.info);
